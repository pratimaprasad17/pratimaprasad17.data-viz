 ...
 //   worldRegion_dropdown.addEventListener("change", updateChart);
   ...
   //     updateCountries();
    ...
    function updateChart(){
        ...
        var oldxColumn = d3.select("#xAxisDropdown").property("previousValue");
    ...
    //    console.log("selectedCountries : "+selectedCountries);
        ...
        resultDataSet = filteredData.map(function (item) {
            return { region: item.region, country: item.country, year: item.year, [xColumn]: item[xColumn], [oldxColumn]: item[oldxColumn], [sizeColumn]: item[sizeColumn], imgURL: item.imgURL, oldX: item.x, oldY: item.y};
        });
    ...
    //    circles.exit().remove();
        ...
        var newCircles = circles
            ...
            // newCircles.on("mouseover", onMouseOver)
            // .on("mousemove", onMouseMove)
            // .on("mouseout", onMouseOut);
