   ...
   //     updateCountries();
    ...
    //    console.log("selectedCountries : "+selectedCountries);
        ...
        var resultDataSet = filteredData.map(function (item) {
            return { region: item.region, country: item.country, year: item.year, [xColumn]: item[xColumn], [sizeColumn]: item[sizeColumn], imgURL: item.imgURL, oldX: item.x, oldY: item.y};
        });
    ...
    //    circles.exit().remove();
        ...
        sizeKeyBox.text("");
        sizeKeyBox.append("tspan")
            .text("Min : "+newMin+" || Max : "+newMax)
            // .style("font-size", "15px")
            // .style("color", "white")
            // .style("background-color", "#980046")
            // .style("padding", "2px")
            ;
